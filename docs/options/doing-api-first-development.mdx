...
### Kickstart the persistence model with JDL

API-first development does not replace JHipster's entity generator. A common workflow is to let JDL create the database, repositories, services, DTO mappers, and tests, then let OpenAPI Generator own the public REST contract.

For example, the Pet Store sample can start from this JDL:
...
skipClient for Pet, Category
```

Run it with:

```bash
jhipster jdl petstore.jdl
```

This gives the API-first implementation a real domain model (`Pet`, `Category`), repositories, service classes, and MapStruct entity DTOs. The `enableSwaggerCodegen true` option enables the OpenAPI Generator integration, and `skipClient` avoids generating CRUD screens when the HTTP surface is meant to be driven by the OpenAPI contract.
...
### Keep the OpenAPI delegate as the public REST layer

When entities are generated from JDL, JHipster also generates the usual `web.rest` resources for those entities. If the OpenAPI specification is the source of truth for the public API, avoid exposing two controllers for the same concept.

JHipster does not currently provide a JDL option that generates the server-side entity model while suppressing only the generated REST resource. For an API-first application, use one of these approaches:

- Keep the generated entity resources only when they intentionally expose a separate CRUD API.
- If the OpenAPI paths replace the entity CRUD API, remove the generated `src/main/java/.../web/rest/*Resource.java` classes and matching `src/test/java/.../web/rest/*ResourceIT.java` tests after entity generation.
- Keep custom endpoint implementations in `src/main/java/.../web/api`, implementing the generated delegates from `target/generated-sources/openapi`.

With that layout, JHipster's generated domain, repository, service, and mapper layers remain reusable, while `src/main/resources/swagger/api.yml` remains the public API contract.
...
#### Map OpenAPI DTOs to JHipster services with MapStruct

The OpenAPI Generator creates DTOs in the package configured by `modelPackage`, for example `demo.jhipster.service.api.dto.Pet`. If JDL also generated entity DTOs in `demo.jhipster.service.dto`, keep the two DTO layers explicit:

- OpenAPI DTOs model the external contract.
- JHipster entity DTOs model the service layer generated from JDL.
- MapStruct bridges the two where the shapes are not identical.

For the JDL above, a small mapper can translate the generated OpenAPI DTO to the generated JHipster entity DTO before calling `PetService`:

```java
package demo.jhipster.service.api.mapper;

import demo.jhipster.service.api.dto.NewPet;
import demo.jhipster.service.api.dto.Pet;
import demo.jhipster.service.dto.PetDTO;
...
public interface PetApiMapper {
    ...
    Pet toApi(PetDTO dto);

    @Mapping(target = "categoryId", ignore = true)
    @Mapping(target = "categoryName", ignore = true)
    PetDTO toServiceDto(NewPet apiDto);
}
```

Then inject both the generated JHipster service and the API mapper in the delegate implementation:

```java
package demo.jhipster.web.api;

import demo.jhipster.service.PetService;
import demo.jhipster.service.api.dto.NewPet;
import demo.jhipster.service.api.dto.Pet;
import demo.jhipster.service.api.mapper.PetApiMapper;
import java.net.URI;
...
public class PetsApiDelegateImpl implements PetsApiDelegate {
    ...
    public PetsApiDelegateImpl(PetService petService, PetApiMapper petApiMapper) {
    ...
    @Override
    public ResponseEntity<Pet> findPetById(Long id) {
        Optional<Pet> pet = petService.findOne(id).map(petApiMapper::toApi);
    ...
    public ResponseEntity<Pet> addPet(NewPet newPet) {
        Pet pet = petApiMapper.toApi(petService.save(petApiMapper.toServiceDto(newPet)));
        return ResponseEntity.created(URI.create("/api/pets/" + pet.getId())).body(pet);
    }
}
```

This keeps generated code disposable: JDL can regenerate entity services and mappers, OpenAPI Generator can regenerate API interfaces and DTOs, and the hand-written code stays in the API delegate and API mapper packages.
