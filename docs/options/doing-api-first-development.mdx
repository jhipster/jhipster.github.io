...
## End-to-end API-First workflow (Contract First + JDL)

For teams that want a true Contract First workflow while still leveraging JHipster's entity/service scaffolding, the following hybrid approach is a good starting point. The OpenAPI Specification remains the source of truth for HTTP endpoints, and JDL is used to bootstrap the domain and service layers.

### 1) Model the domain with JDL (entities, services, MapStruct)

Start with a JDL file to generate entities, repositories, services, and mappers. See the [JDL intro](/jdl/intro) and [JDL options](/jdl/options) for details.
...
Generate the domain layer from the JDL (server only):

```bash
jhipster jdl ./model.jdl --skip-client
```
...
This makes it easier to reuse JHipster's DTO conventions and MapStruct tooling.
...
### 5) Implement delegates as your HTTP entry points

Treat the generated `*ApiController`/`*ApiDelegate` classes as the only HTTP entry points for your contract. The [Implement the Delegate](#implement-the-delegate) section below applies to each operation: wire services, map DTOs with MapStruct, and keep business logic out of the generated sources.
