...
If you want to have "live reload" of your JavaScript/TypeScript code, you will need to run `npm start`. You can go to the [Using JHipster in development](/development/) page for more information.
...
The "entity" sub-generator will create all the necessary files, and provide a CRUD front-end for each entity (see [Angular project structure](/using-angular/) and [React project structure](/using-react/)). The sub generator can be invoked by running `jhipster entity <entityName> --[options]`. Reference for those options can be found by typing `jhipster entity --help`
...
* [JHipster UML](/jhipster-uml/), which allows you to use an UML editor.
* [JDL Studio](https://start.jhipster.tech/jdl-studio/), our online tool to create entities and relationships using our domain-specific language [JDL](/jdl/intro/).
...
This is the same logic as using the [Spring service sub-generator](/creating-a-spring-service/), so we recommend to read its documentation to have more information.
...
If you want some more complex business behaviors, you might need to add a Spring `@Service` class, using the [service sub-generator](/creating-a-service/).
